...
Matrix4x4 SkeletonEditingPlugin::globalMatrix(int _objectId, int _jointId){
  ...
  emit scriptInfo("globalMatrix( ObjectId, " + QString::number(_jointId) + " )" );
...
Vector SkeletonEditingPlugin::globalTranslation(int _objectId, int _jointId){
  ...
  emit scriptInfo("globalTranslation( ObjectId, " + QString::number(_jointId) + " )" );
...
Vector SkeletonEditingPlugin::localTranslation(int _objectId, int _jointId){
  ...
  emit scriptInfo("localTranslation( ObjectId, " + QString::number(_jointId) + " )" );
...
int SkeletonEditingPlugin::frameCount(int _objectId, int _animationIndex){
  ...
  if ( (_animationIndex < 0) || (_animationIndex > (int)skeleton->animationCount()) ){
    emit log(LOGERR, tr("Invalid animationIndex"));
    return -1;
  }

  emit scriptInfo("frameCount( ObjectId, " + QString::number(_animationIndex) + " )" );
...
/// set active pose
void SkeletonEditingPlugin::setActivePose(int _objectId, int _animationIndex, int _frame){
  BaseObjectData* obj = 0;
  ...
  if ( (_frame < 0) || (_frame > (int)skeleton->animation( _animationIndex )->frameCount()) ){
    emit log(LOGERR, tr("Invalid frame"));
    return;
  }

  skeletonObj->skeletonNode()->setActivePose( AnimationHandle(_animationIndex,_frame) );
  emit updatedObject(_objectId, UPDATE_GEOMETRY);

  emit scriptInfo("setActivePose( ObjectId, " + QString::number(_animationIndex) + ", " + QString::number(_frame) + ")" );
}

//------------------------------------------------------------------------------

/// add animation
void SkeletonEditingPlugin::addAnimation(int _objectId, QString _name, int _frames){
  BaseObjectData* obj = 0;
  ...
  FrameAnimationT<ACG::Vec3d>* animation = new FrameAnimationT<ACG::Vec3d>(skeleton, _frames);
